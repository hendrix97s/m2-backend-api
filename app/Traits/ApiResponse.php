...
Trait ApiResponse
{
     ...
     */
    protected function response(string $message, array|bool|object $data = [], $statusCode = null, $errorCode = null): Response|Application|ResponseFactory
    {
        if(!$data){
            $message  = __($message . '.fail');
            $response = $this->content($message, [], false, $statusCode, $errorCode);
        }else{
